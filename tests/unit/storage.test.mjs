...
test('a fresh store is empty in every compartment', () => {
  ...
  for(const key of ['words','ex','rw','read','lesson','rsched','log','clozeSeen','clozeStats','knew']){
...
test('load() survives having no back end at all and still returns a usable shape', async () => {
  ...
  for(const key of ['words','ex','rw','read','lesson','rsched','log','clozeSeen','clozeStats','knew']) assert.ok(s[key]);
